...
def main():
    ...
    # =====================================================
    # MODEL SETUP (Standard CrossEntropy - Sanity Check)
    # =====================================================
    model = FraudGNN(in_channels=X_full.shape[1]).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.01, weight_decay=1e-4)

    # CrossEntropy with 15x penalty for fraud (the "sweet spot")
    # 29x was causing collapse; 5x was too weak; 15x should balance
    weights = torch.tensor([1.0, 15.0]).to(device)
    criterion = torch.nn.CrossEntropyLoss(weight=weights)

    print("Starting Sanity Check Training (CrossEntropy, 5x Weight)...")
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 5 == 0:
            ...
            print(
                f"{epoch+1:>5} | {avg_loss:>8.4f} | {tnr*100:>7.2f}% | {tpr*100:>7.2f}% | {f1*100:>7.2f}% | {gmeans*100:>7.2f}%"
            )
