...
def print_training_progress(history):
    ...
    for h in history:
        if h['epoch'] % 10 == 0 or h['epoch'] == len(history) - 1:
            ...
            print(f"| {h['epoch']+1:>5} | {h['train_loss']:>8.4f} | {spec:>7.2f}% | {recall:>7.2f}% | {f1:>7.2f}% | {gmeans:>7.2f}% |")
...
def print_final_results(metrics, latency, train_time, baseline_gmeans=None):
    ...
    print("\nPerformance Metrics:")
    print(f"  Specificity:          {metrics['specificity']*100:>6.2f}%  (CV: {CV_CLAIMS['specificity']}%)")
    ...
    if baseline_gmeans:
        ...
        print(f"  G-Means Improvement:  {improvement:>6.2f}%  (CV: {CV_CLAIMS['gmeans_improvement']}%)")
...
def print_cv_comparison(metrics, latency, baseline_gmeans=None):
    ...
    print("-" * 70)
    print(f"| {'Metric':<20} | {'Achieved':>12} | {'CV Claim':>12} | {'Delta':>8} | {'Status':>6} |")
    print(f"|{'-'*22}|{'-'*14}|{'-'*14}|{'-'*10}|{'-'*8}|")
    
    # Specificity
    delta_spec = spec - CV_CLAIMS['specificity']
    status_spec = "✓" if abs(delta_spec) <= 3 else "✗"
    print(f"| {'Specificity':<20} | {spec:>11.2f}% | {CV_CLAIMS['specificity']:>11.2f}% | {delta_spec:>+7.2f}% | {status_spec:>6} |")
    
    # G-Means Improvement
    delta_gm = gmeans_imp - CV_CLAIMS['gmeans_improvement']
    status_gm = "✓" if abs(delta_gm) <= 5 else "✗"
    print(f"| {'G-Means Improve':<20} | {gmeans_imp:>11.2f}% | {CV_CLAIMS['gmeans_improvement']:>11.2f}% | {delta_gm:>+7.2f}% | {status_gm:>6} |")
...
def save_metrics_csv(history, path="logs/training_metrics.csv"):
    ...
    with open(path, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=[
            ...
            'val_specificity', 'val_recall', 'val_f1', 'val_gmeans'
        ])
        writer.writeheader()
        for h in history:
            writer.writerow({
