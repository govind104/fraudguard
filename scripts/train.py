...
def parse_args():
    ...
    parser.add_argument(
        "--logs_dir", type=str, default="./logs", help="Directory for logs and MLflow tracking"
    )
    parser.add_argument(
        "--sample_frac", type=float, default=1.0, help="Fraction of data to use (0.0-1.0)"
    )

    # Training settings
    parser.add_argument("--epochs", type=int, default=30, help="Number of training epochs")
    parser.add_argument(
        "--batch_size", type=int, default=2048, help="Batch size for NeighborLoader"
    )
    parser.add_argument("--lr", type=float, default=0.003, help="Learning rate")
    parser.add_argument(
        "--fraud_weight", type=float, default=25.0, help="Class weight for fraud (minority class)"
    )
    parser.add_argument(
        "--gradient_clip", type=float, default=1.0, help="Max gradient norm for clipping"
    )

    # MCD settings
    parser.add_argument(
        "--baseline_alpha", type=float, default=0.0, help="MCD alpha for baseline (0.0 = no MCD)"
    )
    parser.add_argument("--gold_alpha", type=float, default=0.80, help="MCD alpha for AD-RL-GNN")

    # Misc
    parser.add_argument("--seed", type=int, default=42, help="Random seed for reproducibility")
    parser.add_argument(
        "--skip_baseline", action="store_true", help="Skip baseline training (only train AD-RL-GNN)"
    )
        ...
        if mlflow:
            ...
            if use_rl:
                ...
                mlflow.pytorch.log_model(
                    model, "fraud_gnn", registered_model_name="FraudGuard-Production"
                )
...
def print_final_comparison(baseline_result, gold_result, evaluator):
    ...
    print(f"|{'-'*17}|{'-'*14}|{'-'*14}|{'-'*14}|")
    print(
        f"| {'G-Means':<15} | {baseline_gmeans*100:>10.2f}% | {gold_gmeans*100:>10.2f}% | {improvement:>+10.1f}% |"
    )
    print(
        f"| {'P95 Latency':<15} | {baseline_p95:>10.2f}ms | {gold_p95:>10.2f}ms | {((gold_p95-baseline_p95)/baseline_p95)*100:>+10.1f}% |"
    )
    print(
        f"| {'Peak VRAM':<15} | {baseline_vram:>10.2f}GB | {gold_vram:>10.2f}GB | {((gold_vram-baseline_vram)/baseline_vram)*100:>+10.1f}% |"
    )
    print("=" * 60)
