...
class FraudTrainer:
    ...
    def fit(
        ...
        logger.info("Training complete", time=f"{train_time:.1f}s", epochs=len(self.history))
    ...
    def _build_graph(self):
        ...
        self.graph_builder.fit(self.X_train)
    ...
    def _prepare_labels(self, train_df, val_df, test_df):
        ...
        logger.info(
            f"Labels: {self.fraud_nodes.shape[0]} fraud, {self.majority_nodes.shape[0]} non-fraud"
        )
    ...
    def _train_rl_and_enhance(self):
        ...
        if hasattr(self, "kept_majority") and self.kept_majority is not None:
            ...
            mask = torch.isin(self.edge_index[0], combined) & torch.isin(
                self.edge_index[1], combined
            )
    ...
    def benchmark_latency(self, n_runs: int = 100) -> Dict[str, float]:
        ...
        return self.evaluator.benchmark_latency(self.model, self.X_full, self.edge_index, n_runs)
