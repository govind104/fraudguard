...
class ExplanationResponse(BaseModel):
    ...
    top_features: List[FeatureAttribution] = Field(..., description="Top contributing features")
    subgraph_nodes: List[int] = Field(..., description="Related transaction nodes")
    edge_importance: Dict[str, float] = Field(
        default_factory=dict, description="Importance of graph connections"
    )
...
class PredictionResponse(BaseModel):
    ...
    explanation: Optional[ExplanationResponse] = Field(
        None, description="Feature attributions (if requested)"
    )
    processing_time_ms: float = Field(..., description="Inference latency in milliseconds")
    model_version: str = Field(default="v1.0.0", description="Model version used for prediction")
...
class BatchPredictionRequest(BaseModel):
    ...
    transactions: List[TransactionRequest] = Field(
        ..., max_length=100, description="List of transactions (max 100)"
    )
    include_explanations: bool = Field(
        default=False, description="Include GNNExplainer attributions"
    )
